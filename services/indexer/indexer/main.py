...
class IndexerConfig(BaseSettings):
    ...
    index_save_interval: int = Field(
        default=5000, env="INDEX_SAVE_INTERVAL"
    )  # Save every N documents
...
class IncrementalTfIdfIndexer:
    ...
    def add_documents_batch(self, batch_data: List[Dict]) -> None:
        ...
        with self.lock:
            ...
            log.info(
                f"Added batch of {len(batch_data)} documents. Total: {self.document_count}"
            )

    def build_final_tfidf_index(
        self,
        ...
        with self.lock:
            ...
            log.info(
                f"Building final TF-IDF index from {len(self.document_texts)} documents"
            )
    ...
    def save_intermediate_index(self, output_path: Path) -> None:
        ...
        with self.lock:
            ...
            joblib.dump(
                self.hash_matrix, output_path / "intermediate_hash_matrix.joblib"
            )

            df = pd.DataFrame(
                {"url": self.document_urls, "title": self.document_titles}
            )
            ...
            with open(output_path / "progress.json", "w") as f:
                json.dump(
                    {"document_count": self.document_count, "timestamp": time.time()}, f
                )
...
class ParallelIndexer:
        ...
        while not self.stop_event.is_set():
            try:
                ...
                if batch_data:
                    ...
                    if (
                        self.processed_count - self.last_save_count
                    ) >= self.config.index_save_interval:
    ...
    def start_workers(self):
        ...
        for i in range(self.config.num_worker_threads):
            worker = threading.Thread(
                target=self._preprocessing_worker, name=f"PreprocessWorker-{i}"
            )
            worker.daemon = True
            worker.start()
    ...
    def build_and_save_final_index(self) -> int:
        ...
        try:
            ...
            tfidf_matrix, feature_names, df, curr_count = (
                self.indexer.build_final_tfidf_index()
            )
            ...
            df[["url", "title"]].to_csv(
                output_path / "document_metadata.csv", index=False
            )

            # Save vectorizer
            joblib.dump(
                self.indexer.tfidf_vectorizer, output_path / "vectorizer.joblib"
            )

            # Clean up intermediate files
            for intermediate_file in [
                "intermediate_hash_matrix.joblib",
                "intermediate_metadata.csv",
                "progress.json",
            ]:
                try:
            ...
            try:
                while True:
                    if config.max_docs and message_count >= config.max_docs:
                        log.info(
                            f"Reached maximum document count ({config.max_docs}), stopping consumption"
                        )
                        break
                    ...
                    try:
                        ...
                        if indexer.processed_count - last_checkpoint >= 100:
                            log.info(
                                f"Checkpoint: Consumed {message_count} messages, processed {indexer.processed_count} documents"
                            )
                ...
                if indexer.processed_count > 0:
                    ...
                    log.info(
                        f"Parallel indexing completed successfully. Total documents: {indexer.processed_count}"
                    )
                else:
