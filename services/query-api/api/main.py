...
class APIConfig(BaseSettings):
    ...
    # API settings
    search_result_limit: int = Field(default=100, env="SEARCH_RESULT_LIMIT")
...
async def search(
    ...
    if not cached_result:
        try:
            ...
            # sort results on the score basis in descending order
            ...
            try:
                redis_client.setex(
                    cache_key, config.redis_cache_ttl, json.dumps(response)
                )
            except Exception as e:
